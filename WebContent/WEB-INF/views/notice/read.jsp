...
<script type="text/javascript">
	...
	$(document).ready(	function() {
		var url = ctx + '/notice/read.do?page=' + ${currentPage} + '&boardSeq=' + ${boardSeq};
							...
							<c:forEach items="${attFiles}" var="file" varStatus="f">
								...
								<tr>
									...
									<td colspan="6" class="tleft"><c:choose>
											...
											<c:otherwise>
												<a
													href="<c:url value='/notice/download.do?fileIdx=${file.fileIdx}'/>">
						...
						<div class="row">
							<div class="col-md-12 text-right">
								<c:if test="${not empty boardList }">
									...
									<!-- 값을 받아오는 걸로 변경. -->
									<a
										href="<c:url value='/notice/goToUpdate.do?page=${currentPage }&boardSeq=${boardList.boardSeq}&title=${boardList.title}&content=${boardList.content}&memberNick=${boardList.memberNick}&hasFile=${boardList.hasFile }' />">
								</c:if>
								...
								<c:choose>
									<c:when test="${empty page}">
										<a href="<c:url value='/notice/list.do?page=1' />">
											<button type="button" class="btn btn-primary">목록</button>
										</a>
									</c:when>
									<c:otherwise>
										<a href="<c:url value='/notice/list.do?page=${page}' />">
											<button type="button" class="btn btn-primary">목록</button>
										</a>
									</c:otherwise>
								</c:choose>
